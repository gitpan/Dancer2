...
template engines, session storage and logging methods and plugings to make
...
=head1 BOOTSTRAPPING (starting a new app)
...
To define a GET action, use the L<get|Dancer2::Manual::DSL/get> keyword.

=item B<POST> The POST method is used to create a resource on the server.

To define a POST action, use the L<post|Dancer2::Manual::DSL/post> keyword.

=item B<PUT> The PUT method is used to replace an existing resource.

To define a PUT action, use the L<put|Dancer2::Manual::DSL/put> keyword.
...
To define a PATCH action, use the L<patch|Dancer2::Manual::Keywoards/patch>
keyword.

=item B<DELETE> The DELETE method requests that the origin server delete the 
resource identified by the Request-URI.

To define a DELETE action, use the L<del|Dancer2::Manual::DSL/del> keyword.
